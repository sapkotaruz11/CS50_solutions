...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        self_copy = copy.deepcopy(self.domains)
        for var in self_copy:
            for x in self_copy[var]:
                if not len(x) == var.length:
    ...
    def revise(self, x, y):
        ...
        revised = False
        self_copy = copy.deepcopy(self.domains)
        overlap_crossword = self.crossword.overlaps[x, y]
        if not overlap_crossword == None:
            i, j = overlap_crossword
            for a in self_copy[x]:
                remove_word = True
                for b in self_copy[y]:
                    if a[i] == b[j] and a != b:
                        remove_word = False
    ...
    def consistent(self, assignment):
        ...
        for var in assignment:
            if assignment[var] in words_assigned:
                return False

            else:
                words_assigned.add(assignment[var])
